...
export class CNodeGForce extends CNode{
    ...
    getValueFrame(f) {
        ...
        var p0 = this.in.source.v(f)
        assert(p0.position !== undefined, "CNodeGForce needs a position track")
        p0 = p0.position
        var p1 = this.in.source.p(f+1)
        var p2 = this.in.source.p(f+2)
        if (this.in.wind) {
        ...
        var s1 = p1.clone().sub(p0)
        var s2 = p2.clone().sub(p1)
        // and one acceleration vector
        var a = s2.clone().sub(s1)
...
export class CNodeRegression extends CNode {
    ...
    recalculate() {
        ...
        this.poly = regression.polynomial(this.array,{order: this.order, precision:16})
...
export function makeMunge(node, index1, index2, scale=1) {
    ...
    return new CNodeMunge({
        ...
        munge: function (f){
            if (index1 === undefined)
                return this.in.n.v(f)
            else if (index2 === undefined)
                return this.in.n.v(f)[index1]
            else
