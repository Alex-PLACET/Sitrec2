...
export class CNodeSAPage extends CNodeDDI {
    ...
    update(frame) {
        ...
        for (var i=0;i<36;i++) {
            ...
            this.pips[i].setPosition(this.cx+this.radius*sin(angle), this.cy-this.radius*cos(angle)+1.5)
        }
    }
    ...
    drawHalfHafu(c, topType, xr, yr, w, h) {
        ...
        if (topType === "Unknown") {
            ...
            if (h<0)
               this.arc(xr, yr, w, 0, 180)
            else
                this.arc(xr, yr, w, 180, 360)
            c.stroke()
    ...
    renderCanvas(frame) {
        ...
        this.hafus.forEach(h => {
            var pos = h.node.p(frame)

            let [xr, yr] = this.worldToSA(pos,true)
            ...
            if (this.showMap) {
                ...
                for (var f = 0; f < Sit.frames; f++) {
                    let [xr, yr] = this.worldToSA(h.node.p(f));
                    if (f === 0) {
                        this.moveTo(xr, yr)
                    } else {
                        this.lineTo(xr, yr)
        ...
        if (this.showMap) {
            ...
            c.beginPath()
            for (var f = 0; f < Sit.frames; f++) {
                let [xr, yr] = this.worldToSA(this.in.jetTrack.p(f));
                if (f === 0) {
                    this.moveTo(xr, yr)
                } else {
                    this.lineTo(xr, yr)
        ...
        if (this.showWind) {
            ...
            const windVelocityScaledLocal = this.in.windLocal.v(par.frame).multiplyScalar(vScale)

            const jetPosition = this.in.jetTrack.p(par.frame);
            const jetVelocityScaled = trackVelocity(this.in.jetTrack, par.frame).multiplyScalar(vScale)
            this.windVelocityVectors(jetPosition, jetVelocityScaled,windVelocityScaledLocal)

            const windVelocityScaledTarget = this.in.windTarget.v(par.frame).multiplyScalar(vScale)

            const gimbalPosition = this.hafus[0].node.p(par.frame);
            const gimbalVelocityScaled = trackVelocity(this.hafus[0].node, par.frame).multiplyScalar(vScale)
            this.windVelocityVectors(gimbalPosition, gimbalVelocityScaled,windVelocityScaledTarget)
    ...
    windVelocityVectors(jetPosition, jetVelocityScaled,windVelocityScaled)
    {
        ...
        let [x0, y0] = this.worldToSA(jetPosition);
        let [x1, y1] = this.worldToSA(groundVelocityEnd);
        let [xa, ya] = this.worldToSA(airVelocityEnd);
